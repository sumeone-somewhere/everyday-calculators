# Everyday Calculators

[![Ganak — Calculations Done, Right](https://ganak.app/opengraph-image.png)](https://ganak.app)

A directory of **249+ free online calculators** for math, finance, health, education, and more — no sign-up required.

All calculators are free to use at [ganak.app](https://ganak.app).

## Why this list?

Most calculator sites are cluttered with ads or force you to create an account. Every tool listed here is:

- **Free** — no paywalls, no premium tiers
- **Instant** — results update as you type
- **Private** — all calculations run in your browser, nothing is sent to a server
- **Mobile-friendly** — works on any screen size

---

## Contents

- [🔢 Math](#math) — [33 calculators](https://ganak.app/calculators/category/math/)
- [📈 Investment](#investment) — [16 calculators](https://ganak.app/calculators/category/investment/)
- [💰 Savings](#savings) — [6 calculators](https://ganak.app/calculators/category/savings/)
- [🏦 Loans & EMI](#loans-emi) — [17 calculators](https://ganak.app/calculators/category/loans-and-emi/)
- [🧾 Tax](#tax) — [20 calculators](https://ganak.app/calculators/category/tax/)
- [💼 Business](#business) — [10 calculators](https://ganak.app/calculators/category/business/)
- [💵 Salary](#salary) — [8 calculators](https://ganak.app/calculators/category/salary/)
- [🏖️ Retirement](#retirement) — [3 calculators](https://ganak.app/calculators/category/retirement/)
- [❤️ Health](#health) — [23 calculators](https://ganak.app/calculators/category/health/)
- [🏋️ Fitness](#fitness) — [4 calculators](https://ganak.app/calculators/category/fitness/)
- [📅 Date & Time](#date-time) — [11 calculators](https://ganak.app/calculators/category/date-and-time/)
- [🏗️ Construction](#construction) — [16 calculators](https://ganak.app/calculators/category/construction/)
- [🔄 Converters](#converters) — [19 calculators](https://ganak.app/calculators/category/converters/)
- [🎓 Academic](#academic) — [9 calculators](https://ganak.app/calculators/category/academic/)
- [🎨 Design](#design) — [3 calculators](https://ganak.app/calculators/category/design/)
- [💻 Tech](#tech) — [6 calculators](https://ganak.app/calculators/category/tech/)
- [🚗 Auto](#auto) — [4 calculators](https://ganak.app/calculators/category/auto/)
- [✈️ Travel](#travel) — [2 calculators](https://ganak.app/calculators/category/travel/)
- [🎲 Fun](#fun) — [3 calculators](https://ganak.app/calculators/category/fun/)
- [🔮 Spirituality](#spirituality) — [16 calculators](https://ganak.app/calculators/category/spirituality/)
- [🎯 Probability](#probability) — [2 calculators](https://ganak.app/calculators/category/probability/)
- [👗 Styling](#styling) — [8 calculators](https://ganak.app/calculators/category/styling/)
- [🏛️ Government Schemes](#government-schemes) — [3 calculators](https://ganak.app/calculators/category/government-schemes/)
- [📦 Others](#others) — [6 calculators](https://ganak.app/calculators/category/others/)
- [📌 Food](#food) — [1 calculator](https://ganak.app/calculators/category/food/)

---

## 🔢 [Math](https://ganak.app/calculators/category/math/)

### 🌐 [Algebra Calculator](https://ganak.app/calculators/algebra-calculator/)

Free algebra calculator with step-by-step solutions. Solve linear equations, quadratic equations (with graph), systems of two equations, and evaluate polynomials. See discriminant, roots, vertex, factored form, and verification. No sign-up required.

### 🌐 [Average Calculator](https://ganak.app/calculators/average-calculator/)

Calculate the average (arithmetic mean), weighted average, median, mode, range, sum, and standard deviation of any set of numbers. Supports both simple and weighted averages with step-by-step formula breakdowns. Essential for grades, statistics, data analysis, and everyday math.

### 🌐 [Circumference Calculator](https://ganak.app/calculators/circumference-calculator/)

Calculate the circumference of a circle from radius, diameter, area, or reverse-calculate any circle property. See step-by-step solutions, results in terms of π, and all circle measurements instantly. Includes formulas for circumference to diameter, area to circumference, and common circle sizes reference table.

### 🌐 [Compound Interest Calculator](https://ganak.app/calculators/compound-interest-calculator/)

Calculate compound interest with multiple compounding frequencies (annually, semi-annually, quarterly, monthly, daily, continuously). See how your money grows exponentially over time with the power of compounding.

### 🌐 [Exponent Calculator](https://ganak.app/calculators/exponent-calculator/)

Calculate any exponent with free step-by-step solutions. Supports positive, negative, zero, and fractional exponents. See results in decimal, fraction, and scientific notation. Includes exponent rules, powers reference table, and common mistakes section.

### 🌐 [Factoring Calculator](https://ganak.app/calculators/factoring-calculator/)

Free factoring calculator with step-by-step solutions. Factor quadratic, cubic, and quartic polynomials using the AC method, difference of squares, sum/difference of cubes, and rational root theorem. Also find all factors, prime factorization, and factor pairs of any number. No sign-up required.

### 🌐 [Fraction Calculator](https://ganak.app/calculators/fraction-calculator/)

Add, subtract, multiply, and divide fractions and mixed numbers with step-by-step solutions. Simplify fractions to lowest terms, convert between decimals and fractions, and see results as fractions, mixed numbers, decimals, and percentages. Includes visual fraction representations and detailed formula breakdowns.

### 🌐 [Golden Ratio Calculator](https://ganak.app/calculators/golden-ratio-calculator/)

Calculate golden ratio segments from any value, generate golden rectangles, create phi-based typography scales, and build Fibonacci-like sequences. Visualize proportions with interactive bars and rectangles. Free online golden ratio calculator for design, architecture, typography, and mathematics.

### 🌐 [Integral Calculator](https://ganak.app/calculators/integral-calculator/)

Free integral calculator with step-by-step solutions. Compute indefinite and definite integrals using power rule, trigonometric, exponential, and substitution techniques. Interactive graph with shaded area, common formulas reference, and numerical integration via adaptive Simpson's rule.

### 🌐 [Interest Rate Calculator](https://ganak.app/calculators/interest-rate-calculator/)

Calculate interest rate, monthly payment, loan amount, or loan term for any fixed-rate loan. Find the interest rate you are being charged, see total interest paid, view a year-by-year amortization schedule, and compare how different rates affect your payments. Works for mortgages, car loans, personal loans, and student loans.

### 🌐 [LCM Calculator](https://ganak.app/calculators/lcm-calculator/)

Find the Least Common Multiple (LCM) of two or more numbers with step-by-step solutions using prime factorization and listing multiples methods. Also shows GCD, prime factors, and the LCM-GCD relationship. Free online LCM calculator for math, fractions, and scheduling problems.

### 🌐 [Log Calculator](https://ganak.app/calculators/log-calculator/)

Free log calculator for any base. Compute logarithms, anti-logs, find the base, and change of base with step-by-step solutions and verification. Includes common log values table, logarithm rules, and real-world applications. Supports log₂, ln, log₁₀, and custom bases.

### 🌐 [Long Division Calculator](https://ganak.app/calculators/long-division-calculator/)

Free long division calculator showing complete step-by-step work. Enter any dividend and divisor to see quotient, remainder, decimal form with repeating detection, and a detailed visual solution process.

### 🌐 [Mean Calculator](https://ganak.app/calculators/mean-calculator/)

Free mean calculator that computes arithmetic, geometric, harmonic, root mean square, and trimmed mean instantly. Enter your numbers and see step-by-step formulas, comparison charts, and the mean inequality relationship. Essential for statistics, data analysis, and math homework.

### 🌐 [Median Calculator](https://ganak.app/calculators/median-calculator/)

Free median calculator. Enter any numbers to find the median, mean, mode, quartiles (Q1, Q3), IQR, and outliers instantly. Shows step-by-step solution for both odd and even datasets with a visual box plot. Essential for statistics, data analysis, and math homework.

### 🌐 [Modulo Calculator](https://ganak.app/calculators/modulo-calculator/)

Free modulo calculator with step-by-step proof and verification. Calculate a mod n instantly, see quotient and remainder, compare math vs. programming conventions, and explore modular arithmetic patterns. Works with negative numbers.

### 🌐 [Percentage Calculator](https://ganak.app/calculators/percentage-calculator/)

All-in-one percentage calculator with 4 calculation modes: find percentage of a number, calculate what percent one value is of another, percentage increase/decrease, and reverse percentage. Essential for discounts, tips, grades, taxes, profit margins, and financial calculations. Includes step-by-step formulas and explanations.

### 🌐 [Percentage Change Calculator](https://ganak.app/calculators/percentage-calculator/change/)

Calculate percentage increase or decrease between two values (old value to new value). Find growth rates, price changes, stock returns, weight loss progress, and year-over-year comparisons. Shows formula: ((New - Old) / Old) × 100. Perfect for business metrics, investment returns, and performance tracking.

### 🌐 [Percentage Difference Calculator](https://ganak.app/calculators/percentage-calculator/difference/)

Calculate the percentage difference between any two numbers using the average method. Compare values like prices, salaries, populations, or measurements. Different from percentage change - this shows the relative difference between two values regardless of which is larger. Formula: \|A - B\| / ((A + B) / 2) × 100.

### 🌐 [Probability Calculator](https://ganak.app/calculators/probability-calculator/)

Free probability calculator with 4 modes: single event, two events (joint, union, conditional, XOR), repeated trials, and Bayes' theorem. See step-by-step solutions with interactive Venn diagrams. Calculate P(A∩B), P(A∪B), P(A\|B), complement, and series probability instantly.

### 🌐 [Pythagorean Theorem Calculator](https://ganak.app/calculators/pythagorean-theorem-calculator/)

Free Pythagorean theorem calculator. Enter any 2 sides of a right triangle to find the missing side, angles, area, and perimeter. Shows step-by-step solution, detects Pythagorean triples, and includes a dynamic diagram. Works with a² + b² = c².

### 🌐 [Quadratic Formula Calculator](https://ganak.app/calculators/quadratic-formula-calculator/)

Solve any quadratic equation using the quadratic formula. Enter coefficients a, b, c to find roots, discriminant, vertex, axis of symmetry, and parabola graph. Shows step-by-step solutions, Vieta's formulas, standard/vertex/factored forms. Handles real and complex roots with exact and decimal answers.

### 🌐 [Ratio Calculator](https://ganak.app/calculators/ratio-calculator/)

Simplify ratios to lowest terms, solve proportions for a missing value, scale ratios up or down, and compare two ratios for equivalence. Supports 2-term and 3-term ratio simplification with step-by-step solutions, visual ratio bars, and results as ratios, decimals, and percentages. Free online ratio calculator for math, cooking, maps, and finance.

### 🌐 [Reverse Percentage Calculator](https://ganak.app/calculators/percentage-calculator/reverse/)

Find the original price or value before a percentage was added or removed. Calculate pre-discount prices, pre-tax amounts, original costs before markup, and base values before percentage changes. Essential for sales tax calculations, discount reverse engineering, and invoice verification. Shows the working formula.

### 🌐 [Rounding Calculator](https://ganak.app/calculators/rounding-calculator/)

Round numbers to any decimal place, whole number position, or significant figures. Supports 9 rounding modes including half up, half down, banker’s rounding, ceiling, and floor. See step-by-step explanations and compare results across all rounding methods instantly.

### 🌐 [Simple Interest Calculator](https://ganak.app/calculators/simple-interest-calculator/)

Calculate simple interest, total amount, and daily interest for loans or savings. Enter principal, annual rate, and time in years/months/days to see interest and payoff totals instantly.

### 🌐 [Slope Calculator](https://ganak.app/calculators/slope-calculator/)

Calculate slope from two points, find angle of incline, percentage grade, and line equations in slope-intercept, point-slope, and standard form. Convert between slope, degrees, and percent grade. Interactive graph with step-by-step solutions, distance, midpoint, and parallel/perpendicular slopes.

### 🌐 [Speed Calculator](https://ganak.app/calculators/speed-calculator/)

Calculate speed, distance, or time instantly. Get results in mph, km/h, m/s, knots, and running pace with step-by-step solutions and unit conversions.

### 🌐 [Square Root Calculator](https://ganak.app/calculators/square-root-calculator/)

Calculate square roots, cube roots, and nth roots of any number. Simplify radicals to exact form (e.g., √72 = 6√2), view prime factorization, step-by-step solutions, and identify perfect squares. Supports imaginary numbers and a complete reference table.

### 🌐 [Standard Deviation Calculator](https://ganak.app/calculators/standard-deviation-calculator/)

Calculate population and sample standard deviation, variance, mean, median, quartiles, and Z-scores for any dataset. Enter numbers separated by commas or spaces. Includes step-by-step formulas, distribution histogram, and coefficient of variation. Free online statistics calculator.

### 🌐 [Triangle Calculator](https://ganak.app/calculators/triangle-calculator/)

Free triangle calculator to solve any triangle. Enter 3 known values (sides or angles) to find all missing sides, angles, area, perimeter, height, medians, and more. Supports SSS, SAS, ASA, AAS, and SSA with step-by-step solutions.

### 🌐 [Volume Calculator](https://ganak.app/calculators/volume-calculator/)

Calculate volume for 10 geometric shapes including cubes, cylinders, spheres, cones, and pyramids. Instant results in ft³, m³, liters, and gallons with surface area. Supports imperial and metric units.

### 🌐 [Weighted Average Calculator](https://ganak.app/calculators/weighted-average-calculator/)

Calculate the weighted average of a set of numbers with different weights. Supports general value-weight pairs, grade calculations with credit hours, and percentage-weighted data. See each item's contribution, compare weighted vs simple average, and get step-by-step formula breakdowns. Essential for GPA, grades, portfolio returns, and cost analysis.

[⬆ Back to contents](#contents)

## 📈 [Investment](https://ganak.app/calculators/category/investment/)

### 🌐 [CAGR Calculator](https://ganak.app/calculators/cagr-calculator/)

Calculate compound annual growth rate (CAGR) for any investment. Switch between 4 modes: find CAGR, project end value, find starting value, or calculate years needed. Includes growth chart, year-by-year breakdown, and real-world examples for stocks, mutual funds, and business revenue.

### 🌐 [Dividend Calculator](https://ganak.app/calculators/dividend-calculator/)

Estimate dividend income, DRIP growth, and yield on cost with dividend and price growth plus contributions.

### 🌐 [FIRE Calculator](https://ganak.app/calculators/fire-calculator/)

Calculate your FIRE number for financial independence. See Lean, Fat, Coast & Barista FIRE targets, year-by-year projections, savings rate analysis, and milestone timeline. Works with any global currency.

### 🌐 [HLV Calculator](https://ganak.app/calculators/hlv-calculator/)

Calculate your Human Life Value (HLV) to find the ideal life insurance coverage. Uses the DIME method with income replacement, outstanding debts, future goals, and existing savings. See year-by-year income projection, coverage gap analysis, and personalized insurance recommendations.

### 🌐 [Inflation Calculator](https://ganak.app/calculators/inflation-calculator/)

Calculate cumulative and average inflation, adjust money across years, and see real value across USD, EUR, GBP, INR, CAD, AUD, JPY, CHF, CNY, SGD, AED.

### 🌐 [NPV Calculator](https://ganak.app/calculators/npv-calculator/)

Calculate Net Present Value (NPV), IRR, Profitability Index, and payback period for investment projects. Compare equal or unequal cash flows, view year-by-year discounted breakdown, and make data-driven investment decisions.

### 🌐 [Rent vs Buy Calculator](https://ganak.app/calculators/rent-vs-buy-calculator/)

Compare the true cost of renting vs buying a home. Factors in mortgage, taxes, maintenance, opportunity cost, and investment returns to find your breakeven point.

### 🌐 [ROI Calculator](https://ganak.app/calculators/roi-calculator/)

Calculate ROI, annualized return (CAGR), real return after inflation, and benchmark outperformance for any global investment scenario.

### 🌐 [Rule of 72 Calculator](https://ganak.app/calculators/rule-of-72-calculator/)

Calculate how long your investment takes to double using the Rule of 72. Compare estimated vs exact doubling time, view rate comparisons, and project investment growth instantly.

### 🌐 [XIRR Calculator](https://ganak.app/calculators/xirr-calculator/)

Calculate XIRR (Extended Internal Rate of Return) for SIP, mutual funds, or any investment with irregular cash flows. Supports regular SIP mode and custom cash flow entry. See annualized returns, net gain, absolute return, and cash flow timeline chart.

### 🇮🇳 [IDCW Calculator](https://ganak.app/calculators/india/idcw-calculator/)

Calculate IDCW (Income Distribution cum Capital Withdrawal) payouts, tax impact, and compare IDCW vs Growth mutual fund plans. See NAV erosion, net income after tax, effective yield, and which option builds more wealth over time.

### 🇮🇳 [Lumpsum Calculator](https://ganak.app/calculators/india/lumpsum-calculator/)

Calculate returns on one-time lumpsum mutual fund investments in India. Estimate future value, total gains, and absolute returns using compound interest. Compare lumpsum vs SIP returns, plan for goals like retirement, home purchase, or child education. Includes year-by-year growth projection and inflation-adjusted values.

### 🇮🇳 [Mutual Fund Calculator](https://ganak.app/calculators/india/mutual-fund-calculator/)

Calculate mutual fund returns for SIP, lumpsum, or goal-based investments in India. Estimate maturity value, total gains, and wealth multiple. Includes step-up SIP, year-by-year growth projection, and visual breakdown of invested amount vs returns. Plan for goals like retirement, child education, or home purchase.

### 🇮🇳 [SIP Calculator](https://ganak.app/calculators/india/sip-calculator/)

Calculate SIP (Systematic Investment Plan) returns for mutual funds in India. Estimate maturity value, total invested amount, and wealth gained over time. Compare different SIP amounts and tenures, plan for financial goals. Includes step-up SIP option, exit load calculations, and LTCG tax estimation for equity funds.

### 🇮🇳 [Step Up SIP Calculator](https://ganak.app/calculators/india/step-up-sip-calculator/)

Calculate Step-Up SIP returns with annual investment increases. See how increasing your SIP by 5-15% yearly can double your wealth compared to regular SIP. Essential for salary growth planning, inflation beating, and accelerated wealth creation. Includes comparison charts showing step-up vs regular SIP growth over 10-30 years.

### 🇮🇳 [SWP Calculator](https://ganak.app/calculators/india/swp-calculator/)

Calculate Systematic Withdrawal Plan returns for mutual funds in India. Estimate remaining corpus, total withdrawals, and returns earned. Plan retirement income with step-up withdrawals, corpus depletion timeline, and year-by-year breakdown.

[⬆ Back to contents](#contents)

## 💰 [Savings](https://ganak.app/calculators/category/savings/)

### 🌐 [APY Calculator](https://ganak.app/calculators/apy-calculator/)

Calculate Annual Percentage Yield (APY) and compare APR vs APY for savings accounts, CDs, and high-yield accounts. Project savings growth with daily, monthly, or annual compounding and recurring contributions. See how compounding frequency impacts your returns.

### 🌐 [Emergency Fund Calculator](https://ganak.app/calculators/emergency-fund-calculator/)

Calculate your ideal emergency fund size based on monthly expenses, job stability, dependents, and financial obligations. Find how many months of expenses you need (3-12 months), track savings progress, and estimate time to reach your goal. Essential for financial security, unexpected expenses, and job loss protection. Includes investment recommendations for parking emergency funds.

### 🌐 [Savings Goal Calculator](https://ganak.app/calculators/savings-goal-calculator/)

Free savings goal calculator. Find how much to save monthly, biweekly, or weekly to reach your target. Includes compound interest, multiple currencies, and visual growth projections. Plan for emergency funds, vacations, down payments, education, and more.

### 🇺🇸 [CD Calculator](https://ganak.app/calculators/us/cd-calculator/)

Free CD calculator. Calculate certificate of deposit interest, maturity amount, and APY. Compare compounding frequencies (daily, monthly, quarterly), estimate early withdrawal penalties, and plan CD ladder strategies. See how your deposit grows with compound interest over any term.

### 🇮🇳 [FD Calculator](https://ganak.app/calculators/india/fd-calculator/)

Calculate Fixed Deposit (FD) maturity amount and interest earned for Indian banks. Compare FD rates across SBI, HDFC, ICICI, and other banks. Supports quarterly, monthly, and cumulative interest payout options. Essential for safe investment planning, senior citizen FD rates, and tax-saving 5-year FDs under Section 80C.

### 🇮🇳 [RD Calculator](https://ganak.app/calculators/india/rd-calculator/)

Calculate Recurring Deposit (RD) maturity amount, total interest earned, and month-by-month growth. Compare RD returns across Indian banks, plan for short-term savings goals, and understand compounding benefits. Perfect for disciplined monthly savings, building emergency funds, and achieving specific financial targets.

[⬆ Back to contents](#contents)

## 🏦 [Loans & EMI](https://ganak.app/calculators/category/loans-and-emi/)

### 🌐 [Amortization Calculator](https://ganak.app/calculators/amortization-calculator/)

Free amortization calculator with full payment schedule. See monthly principal and interest breakdown for mortgages, auto loans, personal loans, and student loans. Calculate extra payment savings, visualize the cross-over point, and generate yearly or monthly amortization tables. Supports bi-weekly payments and one-time lump sum prepayments.

### 🌐 [Auto Lease Calculator](https://ganak.app/calculators/auto-lease-calculator/)

Free auto lease calculator. Calculate your auto lease payment with money factor, residual value, and cap cost breakdown. Includes lease vs buy comparison and full payment schedule.

### 🌐 [Car Lease Calculator](https://ganak.app/calculators/car-lease-calculator/)

Free car lease calculator. Estimate your monthly car lease payment with depreciation, finance charge, and tax breakdown. Compare lease vs buy, view payment schedule, and calculate due-at-signing costs.

### 🌐 [Lease Calculator](https://ganak.app/calculators/lease-calculator/)

Free lease calculator. Calculate monthly lease payment, total cost, and compare lease vs buy. Enter vehicle or equipment price, residual value, money factor or APR, and lease term. Includes payment breakdown, schedule, and multi-currency support.

### 🌐 [Loan Amortization Calculator](https://ganak.app/calculators/loan-amortization-calculator/)

Generate a complete loan amortization schedule for any loan type. See how each payment is split between principal and interest, calculate the impact of extra payments, and find your cross-over point. Works for mortgages, car loans, personal loans, and student loans.

### 🌐 [Mortgage Amortization Calculator](https://ganak.app/calculators/mortgage-amortization-calculator/)

Calculate your mortgage amortization schedule with extra payments. See how each monthly payment breaks down into principal and interest, find the cross-over point, and visualize your loan payoff timeline. Supports bi-weekly payments for faster mortgage payoff.

### 🇮🇳 [Car Loan EMI Calculator](https://ganak.app/calculators/india/car-loan-emi-calculator/)

Free car loan EMI calculator for India. Calculate monthly EMI, total interest, and view complete amortization schedule for new and used car loans. Compare EMIs across tenures from 1 to 7 years. Includes bank-wise interest rate comparison, prepayment analysis, and processing fee calculation. Works for SBI, HDFC, ICICI, Axis Bank, and all major lenders.

### 🇺🇸 [Car Payment Calculator](https://ganak.app/calculators/us/car-payment-calculator/)

Free car payment calculator. Estimate your monthly auto loan payment with down payment, trade-in value, sales tax, and interest rate. Compare loan terms from 12 to 96 months side by side. Includes amortization schedule, negative equity handling, and extra payment savings analysis for new and used car loans.

### 🇮🇳 [EMI Calculator](https://ganak.app/calculators/india/emi-calculator/)

Calculate EMI (Equated Monthly Installment) for home loans, car loans, personal loans, and education loans in India. View complete amortization schedule with principal and interest breakdown for each month. Compare EMIs across different interest rates and tenures. Essential for loan planning and affordability assessment.

### 🇮🇳 [Home Loan Eligibility Calculator](https://ganak.app/calculators/india/home-loan-eligibility-calculator/)

Calculate your home loan eligibility based on salary, age, existing obligations, and co-applicant income. Uses bank-standard FOIR method for salaried and self-employed applicants. Find out the maximum home loan amount you can get from Indian banks like SBI, HDFC, ICICI, and others.

### 🇮🇳 [Home Loan EMI Calculator](https://ganak.app/calculators/india/home-loan-emi-calculator/)

Calculate home loan EMI with complete tax benefit analysis under Section 80C (₹1.5 lakh deduction on principal) and Section 24(b) (₹2 lakh deduction on interest). Estimate prepayment impact, compare loan offers, and optimize your home buying budget. Essential for first-time home buyers in India.

### 🇮🇳 [House Loan EMI Calculator](https://ganak.app/calculators/india/house-loan-emi-calculator/)

Calculate house/home loan EMI with Indian tax benefits under Section 80C (principal) and Section 24(b) (interest). Estimate prepayment savings, compare floating vs fixed rates, and plan affordable monthly payments. Includes stamp duty, registration costs, and down payment calculations for property purchase planning.

### 🇮🇳 [Loan Calculator](https://ganak.app/calculators/india/loan-calculator/)

Universal loan EMI calculator for any loan type - personal, car, education, business, or gold loans. Calculate monthly payments, total interest payable, and complete repayment schedule. Compare loans from different banks and NBFCs. Includes prepayment impact analysis and loan restructuring scenarios.

### 🇮🇳 [Loan Eligibility Calculator](https://ganak.app/calculators/india/loan-eligibility-calculator/)

Check your loan eligibility instantly based on income, existing EMIs, and loan tenure. Supports home loan, personal loan, car loan, education loan, and loan against property. Uses FOIR (Fixed Obligation to Income Ratio) method with salaried and self-employed profiles. Add co-applicant income to maximize your eligible loan amount.

### 🇮🇳 [Loan Interest Calculator](https://ganak.app/calculators/india/loan-interest-calculator/)

Calculate and compare loan interest costs across different banks and tenures. View total repayment amount, month-by-month interest vs principal breakdown, and find the most affordable loan option. Essential for comparing fixed vs floating rate loans and understanding true cost of borrowing.

### 🇺🇸 [Mortgage Payoff Calculator](https://ganak.app/calculators/us/mortgage-payoff-calculator/)

Calculate your mortgage payoff with bi-weekly payments, extra principal payments, and lump sum options. Visualize how much interest and time you can save with accelerated payoff strategies. Includes PMI, taxes, and insurance for a complete PITI view.

### 🇮🇳 [Personal Loan Eligibility Calculator](https://ganak.app/calculators/india/personal-loan-eligibility-calculator/)

Check your personal loan eligibility based on monthly income and existing EMI obligations. Calculate maximum loan amount you can get from banks and NBFCs in India. Uses FOIR-based methodology for accurate results for both salaried and self-employed applicants.

[⬆ Back to contents](#contents)

## 🧾 [Tax](https://ganak.app/calculators/category/tax/)

### 🌐 [Stamp Duty Calculator AU](https://ganak.app/calculators/au/stamp-duty-calculator/)

Calculate stamp duty (transfer duty) for all Australian states and territories. Covers property and vehicle purchases with first home buyer concessions, foreign buyer surcharges, and state-by-state comparison. Supports NSW, VIC, QLD, WA, SA, ACT, TAS, and NT with current rate tables.

### 🇮🇳 [Capital Gains Tax Calculator](https://ganak.app/calculators/india/capital-gains-tax-calculator/)

Calculate Short Term Capital Gains (STCG) and Long Term Capital Gains (LTCG) tax on investments in India. Covers listed equity shares (20% STCG / 12.5% LTCG), mutual funds, property, gold, cryptocurrency (30% flat), and other assets. Updated for Finance Act 2024 changes. Essential for ITR filing and tax-loss harvesting strategies.

### 🇮🇳 [Crypto Capital Gains Tax Calculator](https://ganak.app/calculators/india/crypto-capital-gains-tax-calculator/)

Calculate tax on cryptocurrency and Virtual Digital Assets (VDAs) in India. Flat 30% tax on all crypto gains regardless of holding period, plus 1% TDS on transfers. No deductions except cost of acquisition. Covers Bitcoin, Ethereum, NFTs, and all crypto tokens under 2022 Budget rules.

### 🇮🇳 [Debt Mutual Funds Capital Gains Tax Calculator](https://ganak.app/calculators/india/debt-mf-capital-gains-tax-calculator/)

Calculate tax on debt mutual funds in India under new 2023 rules. All gains taxed at slab rate regardless of holding period (no LTCG benefit). Covers liquid funds, money market, corporate bond funds, and gilt funds. Important changes from April 2023 affecting debt fund taxation.

### 🇮🇳 [Equity Mutual Funds Capital Gains Tax Calculator](https://ganak.app/calculators/india/equity-mf-capital-gains-tax-calculator/)

Calculate capital gains tax on equity mutual funds in India. STCG at 20% for holdings under 1 year, LTCG at 12.5% above ₹1.25 lakh for holdings over 1 year. Covers equity-oriented funds, ELSS, index funds, and hybrid funds with 65%+ equity. Updated for Finance Act 2024 changes.

### 🇮🇳 [Gold Capital Gains Tax Calculator](https://ganak.app/calculators/india/gold-capital-gains-tax-calculator/)

Calculate capital gains tax on physical gold and jewelry in India. STCG at slab rate for under 2 years, LTCG at 12.5% for over 2 years. Covers gold coins, bars, jewelry, and inherited gold. Updated for Finance Act 2024 — indexation no longer applies.

### 🇮🇳 [Gold ETF/SGB Capital Gains Tax Calculator](https://ganak.app/calculators/india/gold-etf-capital-gains-tax-calculator/)

Calculate tax on Gold ETFs and Sovereign Gold Bonds (SGBs) in India. Gold ETFs use a 12-month holding period — STCG at slab rate, LTCG at 12.5%. SGBs are tax-free on maturity. Compare tax efficiency of different gold investment options for optimal portfolio allocation.

### 🇮🇳 [GST Calculator](https://ganak.app/calculators/india/gst-calculator/)

Calculate GST (Goods and Services Tax) for India with detailed CGST, SGST, and IGST breakdown. Add or remove GST from any amount, switch between tax-inclusive and tax-exclusive pricing. Supports all GST slab rates (5%, 12%, 18%, 28%). Essential for businesses, invoicing, billing software, and GST return filing.

### 🇮🇳 [Income Tax Calculator](https://ganak.app/calculators/india/income-tax-calculator/)

Calculate income tax for FY 2025-26 and FY 2024-25 under both Old and New tax regimes. Compare tax liability, see slab-wise breakdown, and find which regime saves you more. Supports key Old Regime deductions — 80C, 80D (self + parents), HRA, NPS, home loan interest, and more. Updated for Union Budget 2025 with revised new regime slabs and ₹12.75 lakh tax-free income for salaried individuals.

### 🇮🇳 [Listed Equity Capital Gains Tax Calculator](https://ganak.app/calculators/india/listed-equity-capital-gains-tax-calculator/)

Calculate STCG (20%) and LTCG (12.5% above ₹1.25 lakh exemption) on listed equity shares in India. Covers Nifty 50, stocks, and equity-oriented ETFs held on recognized stock exchanges. Updated for Finance Act 2024 with revised rates and LTCG exemption limit. Essential for stock traders and long-term equity investors.

### 🇮🇳 [Other Assets Capital Gains Tax Calculator](https://ganak.app/calculators/india/other-assets-capital-gains-tax-calculator/)

Calculate capital gains tax on other assets including collectibles, artwork, antiques, and miscellaneous investments in India. General rules: STCG at slab rate, LTCG at 12.5% for assets held over 2 years. Customize holding period and asset type for accurate calculation.

### 🇺🇸 [Powerball Jackpot Calculator](https://ganak.app/calculators/us/powerball-jackpot-calculator/)

Calculate your Powerball jackpot payout after federal and state taxes. Compare lump sum vs 30-year annuity, see the full tax breakdown by state, explore the Power Play multiplier, and visualize the investment comparison. Covers all 45 Powerball states plus DC with 2026 tax brackets. Free, instant, and private.

### 🇮🇳 [Property Capital Gains Tax Calculator](https://ganak.app/calculators/india/property-capital-gains-tax-calculator/)

Calculate capital gains tax on real estate property sale in India. LTCG at 12.5% for properties held over 2 years (20% with indexation available for pre-July 2024 purchases). Covers residential, commercial, land, and inherited property. Includes Section 54/54EC exemptions for reinvesting in new property or capital gains bonds.

### 🇮🇳 [REITs Capital Gains Tax Calculator](https://ganak.app/calculators/india/reit-capital-gains-tax-calculator/)

Calculate capital gains tax on REITs (Real Estate Investment Trusts) and InvITs (Infrastructure Investment Trusts) in India. STCG at 20% and LTCG at 12.5% above ₹1.25 lakh. Understand dividend taxation and compare with direct real estate investment for tax efficiency.

### 🇺🇸 [Sales Tax Calculator](https://ganak.app/calculators/us/sales-tax-calculator/)

Calculate US sales tax for any state. Add or remove sales tax from any purchase price. Covers all 50 states with state-level average combined rates for quick estimates. Exact sales tax depends on the full delivery address and item taxability. Essential for e-commerce sellers, shoppers, and businesses operating across multiple US states.

### 🇮🇳 [Stamp Duty Calculator India](https://ganak.app/calculators/in/stamp-duty-calculator/)

Calculate stamp duty and registration charges for property purchases across all 28 Indian states and 8 union territories. Includes gender-based concessions for women buyers, property type variations (residential, commercial, agricultural), metro cess, and state-by-state comparison. Updated with current rates for 2025-26.

### 🇬🇧 [Stamp Duty Calculator UK](https://ganak.app/calculators/uk/stamp-duty-calculator/)

Calculate stamp duty on UK residential property purchases. Covers Stamp Duty Land Tax (SDLT) for England and Northern Ireland, LBTT for Scotland, and LTT for Wales. Updated for April 2025 rate changes with first-time buyer relief, additional property surcharge (+5%), and non-UK resident surcharge (+2%). See band-by-band breakdown, effective rate, and region comparison.

### 🇮🇳 [TDS Calculator](https://ganak.app/calculators/india/tds-calculator/)

Calculate TDS (Tax Deducted at Source) on payments in India. Covers all major TDS sections — 194A (interest), 194C (contractor), 194H (commission), 194I (rent), 194J (professional fees), and more. Also calculate late payment interest under Section 201(1A) and late filing fees under Section 234E. Updated for FY 2024-25 with PAN and non-PAN rates.

### 🇮🇳 [Unlisted Shares Capital Gains Tax Calculator](https://ganak.app/calculators/india/unlisted-shares-capital-gains-tax-calculator/)

Calculate capital gains tax on unlisted shares and startup equity (ESOPs) in India. STCG at slab rate for under 2 years, LTCG at 12.5% for over 2 years. Important for startup employees, angel investors, and pre-IPO shareholders. Includes fair market value considerations.

### 🇬🇧 [VAT Calculator](https://ganak.app/calculators/uk/vat-calculator/)

Calculate UK VAT instantly. Add or remove VAT at the standard rate (20%), reduced rate (5%), or any custom rate. See net price, VAT amount, and gross price breakdown. Free online VAT calculator for businesses, freelancers, and individuals in the United Kingdom.

[⬆ Back to contents](#contents)

## 💼 [Business](https://ganak.app/calculators/category/business/)

### 🌐 [Break Even Calculator](https://ganak.app/calculators/break-even-calculator/)

Calculate your break even point in units and revenue. Determine how many units you need to sell to cover fixed and variable costs, visualize profit zones, and plan target profit scenarios.

### 🌐 [Contribution Margin Calculator](https://ganak.app/calculators/contribution-margin-calculator/)

Calculate contribution margin per unit, contribution margin ratio, and total contribution margin. Enter optional fixed costs to see break-even units and operating income. Understand how much each unit sold contributes to covering fixed costs and generating profit.

### 🌐 [CPM Calculator](https://ganak.app/calculators/cpm-calculator/)

Calculate CPM (Cost Per Mille), impressions, or ad budget from any two inputs. Compare campaigns and optimize your advertising costs with this free CPM calculator.

### 🌐 [Depreciation Calculator](https://ganak.app/calculators/depreciation-calculator/)

Calculate asset depreciation using straight-line, double declining balance, and sum-of-years'-digits methods. View year-by-year schedules, compare methods, and understand book value decline.

### 🌐 [Engagement Rate Calculator](https://ganak.app/calculators/engagement-rate-calculator/)

Calculate your social media engagement rate by followers, reach, or impressions. Compare engagement across Instagram, TikTok, YouTube, Twitter, Facebook, and LinkedIn with platform-specific benchmarks.

### 🌐 [Gross Margin Calculator](https://ganak.app/calculators/gross-margin-calculator/)

Calculate gross margin percentage, gross profit, and markup from revenue and cost of goods sold. Compare your margins to industry benchmarks and understand the difference between margin and markup.

### 🌐 [Margin Calculator](https://ganak.app/calculators/margin-calculator/)

Calculate profit margin, markup, and selling price from cost and revenue. Find the required selling price for a target margin or the maximum cost to maintain profitability. Compare margin vs markup and benchmark against industry averages.

### 🌐 [Markup Calculator](https://ganak.app/calculators/markup-calculator/)

Calculate markup percentage, selling price, or cost price. Essential for retail, wholesale, and business pricing strategies to ensure profitability. Determine the perfect selling price based on your desired markup or profit margin.

### 🌐 [Net Profit Margin Calculator](https://ganak.app/calculators/net-profit-margin-calculator/)

Calculate net profit margin percentage from revenue and all expenses. See gross margin, operating margin, and net margin side by side. Compare your margins to industry benchmarks and understand before-tax vs after-tax profitability.

### 🇺🇸 [PayPal Fee Calculator](https://ganak.app/calculators/paypal-fee-calculator/)

Calculate PayPal fees for Goods & Services, invoices, QR codes, Venmo, Friends & Family, and international transfers. See exact fees, net amount received, effective fee rate, and what to charge clients. Supports all 2026 PayPal transaction types with real-time calculation.

[⬆ Back to contents](#contents)

## 💵 [Salary](https://ganak.app/calculators/category/salary/)

### 🌐 [Commission Calculator](https://ganak.app/calculators/commission-calculator/)

Free commission calculator. Calculate sales commission with flat rate, tiered brackets, or agent/broker splits. See total earnings, per-sale commission, and annual projections with multi-currency support.

### 🌐 [Hourly to Salary Calculator](https://ganak.app/calculators/hourly-to-salary-calculator/)

Free hourly to salary calculator. Convert your hourly wage to annual salary, monthly, or weekly pay instantly. Discover your yearly earnings based on your hourly rate with customizable work hours and weeks. Ideal for pay negotiations, budget planning, and comparing job offers.

### 🌐 [Overtime Pay Calculator](https://ganak.app/calculators/overtime-pay-calculator/)

Free overtime pay calculator. Calculate your overtime earnings with support for time-and-a-half, double time, and custom multipliers. See regular pay, overtime pay, total gross pay, and annual projections for any pay period.

### 🌐 [Salary Hike Calculator](https://ganak.app/calculators/salary-hike-calculator/)

Free salary hike calculator. Calculate your salary hike percentage from old and new salary, or find your new salary after a percentage hike. Supports annual, monthly, bi-weekly, and weekly pay frequencies with full breakdown.

### 🌐 [Salary Increase Calculator](https://ganak.app/calculators/salary-increase-calculator/)

Project your salary growth across multiple years with percentage raises, promotion boosts, and optional inflation adjustments. See final pay, total extra earnings, and real (purchasing-power) salary.

### 🌐 [Salary to Hourly Calculator](https://ganak.app/calculators/salary-to-hourly-calculator/)

Free salary to hourly calculator. Convert your annual salary, monthly pay, or weekly wages to an hourly rate instantly. Find out what your salary works out to per hour with customizable hours per week and weeks per year. Perfect for job comparisons, freelance rate setting, and financial planning.

### 🇮🇳 [Gratuity Calculator](https://ganak.app/calculators/india/gratuity-calculator/)

Free gratuity calculator for India. Calculate your gratuity amount using the formula G = n × b × 15/26. For employees covered and not covered under the Gratuity Act, 1972.

### 🇺🇸 [US Paycheck Calculator](https://ganak.app/calculators/us/paycheck-calculator/)

Free paycheck calculator for all 50 US states. Calculate your take-home pay with federal and state taxes, Social Security, Medicare, 401(k), HSA, and deductions. See per-paycheck and annual breakdowns instantly.

[⬆ Back to contents](#contents)

## 🏖️ [Retirement](https://ganak.app/calculators/category/retirement/)

### 🌐 [Retirement Calculator](https://ganak.app/calculators/retirement-calculator/)

Free retirement calculator to estimate how much you need to save, project your retirement corpus, and calculate sustainable monthly income. Supports multiple currencies worldwide. Covers accumulation phase with compound growth, decumulation with inflation-adjusted withdrawals, the 4% rule for sustainable income, and additional income sources like pensions and social security. Essential for retirement planning at any age.

### 🇺🇸 [401(k) Retirement Calculator](https://ganak.app/calculators/us/401k-retirement-calculator/)

Comprehensive 401(k) retirement calculator for US investors. Project how long your savings will last, calculate sustainable monthly income, Required Minimum Distributions (RMDs), employer match impact, and tax implications. Compare withdrawal strategies including 4% rule, fixed income, and RMD-only approaches. Essential for retirement planning, contribution optimization, and financial independence (FIRE) calculations.

### 🇬🇧 [UK Pension Calculator](https://ganak.app/calculators/uk/pension-calculator/)

Free UK pension calculator. Estimate your retirement income from workplace pension, State Pension, and personal contributions. See PLSA benchmarks, drawdown projections, growth scenarios, and how long your pension pot will last. Includes NI qualifying years, 25% tax-free lump sum, and auto-enrolment comparison.

[⬆ Back to contents](#contents)

## ❤️ [Health](https://ganak.app/calculators/category/health/)

### 🌐 [A1C Calculator](https://ganak.app/calculators/a1c-calculator/)

Convert A1C ↔ average glucose (eAG), estimate A1C from glucose readings, GMI, and fructosamine. Includes mg/dL ↔ mmol/L conversion, charts, and A1C range interpretation.

### 🌐 [BAC Calculator](https://ganak.app/calculators/bac-calculator/)

Free BAC calculator using the Widmark formula. Estimate your blood alcohol content by weight, gender, and drinks consumed. See your current BAC, peak BAC, time to sober, impairment level, and an interactive timeline chart. Includes standard drink calculator, effects at each BAC level, and legal limits.

### 🌐 [BMI Calculator](https://ganak.app/calculators/bmi-calculator/)

Calculate your Body Mass Index (BMI) using height and weight. Instantly find your BMI category (underweight, normal, overweight, obese) based on WHO and CDC standards. Includes BMI chart, healthy weight range, and personalized health recommendations. Supports metric (kg/cm) and imperial (lbs/inches) units.

### 🌐 [BMR Calculator](https://ganak.app/calculators/bmr-calculator/)

Calculate your Basal Metabolic Rate (BMR) - the calories your body burns at complete rest. Uses Mifflin-St Jeor and Harris-Benedict equations for accurate results. Essential for weight loss, muscle gain, and nutrition planning. Find your daily calorie needs with activity level multipliers for sedentary to very active lifestyles.

### 🌐 [Body Fat Calculator](https://ganak.app/calculators/body-fat-calculator/)

Estimate body fat percentage using the U.S. Navy method and BMI-based calculations. Compare fat mass vs lean mass, view your body composition category, and track fitness progress. Requires waist, neck, and hip measurements. Includes body fat category charts for men and women, plus healthy body fat ranges by age.

### 🌐 [Calorie Calculator](https://ganak.app/calculators/calorie-calculator/)

Free calorie calculator to find your daily calorie needs. Calculate maintenance calories, calorie deficit for weight loss, or surplus for muscle gain. Uses Mifflin-St Jeor and Katch-McArdle formulas. Includes BMR, TDEE, macronutrient breakdown, and goal-based calorie targets with weight change projections.

### 🌐 [Calorie Deficit Calculator](https://ganak.app/calculators/calorie-deficit-calculator/)

Free calorie deficit calculator for weight loss. Enter your stats and goal weight to get your daily calorie target, deficit amount, and estimated timeline. Includes weight loss projection chart, macronutrient breakdown, 7-day calorie cycling plan, and multi-deficit comparison. Uses Mifflin-St Jeor and Katch-McArdle formulas.

### 🌐 [Creatinine Clearance Calculator](https://ganak.app/calculators/creatinine-clearance-calculator/)

Calculate creatinine clearance (CrCl) using the Cockcroft-Gault equation with automatic ideal body weight (IBW) and adjusted body weight (ABW) selection. Includes renal function classification (normal, mild, moderate, severe impairment) and drug dosing thresholds. Supports mg/dL and µmol/L. Free, instant results calculated in your browser.

### 🌐 [eGFR Calculator](https://ganak.app/calculators/egfr-calculator/)

Free eGFR calculator to estimate your kidney function using the CKD-EPI 2021, CKD-EPI 2009, MDRD, and Cockcroft-Gault equations. Enter serum creatinine, age, and sex for instant results with CKD staging (KDIGO G1–G5), risk classification, and all-equation comparison. Supports mg/dL and µmol/L creatinine units.

### 🌐 [FFMI Calculator](https://ganak.app/calculators/ffmi-calculator/)

Calculate your Fat-Free Mass Index (FFMI) to assess your muscularity and natural genetic potential. Get your normalized FFMI, body composition breakdown, classification category, and progress toward the natural limit. Based on Kouri et al. (1995) research. Supports metric and imperial units.

### 🌐 [Height Calculator](https://ganak.app/calculators/height-calculator/)

Predict your child's adult height using 3 scientific methods (Khamis-Roche, Mid-Parental, CDC multiplier). Check height percentile against CDC standards for any age and gender. Convert between cm, feet & inches, meters, and inches. Includes method comparison with confidence intervals, growth remaining, and percentile visualization.

### 🌐 [Ideal Weight Calculator](https://ganak.app/calculators/ideal-weight-calculator/)

Calculate your ideal body weight using multiple scientifically-validated formulas (Devine, Robinson, Miller, Hamwi). Find your healthy weight range based on height, age, frame size, and gender. Perfect for fitness goal setting, weight loss targets, and health assessments. Includes BMI-based recommendations and ideal weight ranges.

### 🌐 [Life Expectancy Calculator](https://ganak.app/calculators/life-expectancy-calculator/)

Calculate your estimated life expectancy based on age, gender, country, lifestyle habits, and health factors. Includes survival probability curve, health age comparison, factor impact breakdown, and personalized recommendations. Uses WHO and SSA actuarial data with evidence-based adjustments for smoking, exercise, BMI, diet, sleep, stress, and more.

### 🌐 [Macro Calculator](https://ganak.app/calculators/macro-calculator/)

Free macro calculator to find your ideal daily protein, carbs, and fat intake. Calculate macronutrients based on your body stats, activity level, and fitness goals. Choose from preset diet plans (Balanced, Low Carb, High Protein, Keto, Low Fat) or create custom macro ratios. Includes per-meal breakdown, diet plan comparison, and personalized protein recommendations. Uses Mifflin-St Jeor and Katch-McArdle formulas.

### 🌐 [Ovulation Calculator](https://ganak.app/calculators/ovulation-calculator/)

Free ovulation calculator to find your most fertile days and predict ovulation dates. Enter your last period date and cycle length to see a 6-month fertility calendar with estimated ovulation dates, fertile windows, and due dates if conceived. Includes cycle phase timeline, conception probability chart, and signs of ovulation. Supports irregular cycles (21-45 days).

### 🌐 [Period Calculator](https://ganak.app/calculators/period-calculator/)

Free period calculator to predict your next period date, safe and unsafe days, fertile window, ovulation, and PMS timing. Enter your last period date, cycle length, and period duration for a 6-month cycle calendar with period predictions, safe period windows, and calendar export. Supports cycles from 21 to 45 days.

### 🌐 [Pregnancy Due Date Calculator](https://ganak.app/calculators/pregnancy-due-date-calculator/)

Free pregnancy due date calculator to estimate when your baby will arrive. Calculate your estimated due date using last menstrual period (LMP), conception date, IVF transfer date, or ultrasound. Includes gestational age, trimester timeline, pregnancy milestones, and week-by-week progress tracker. Uses Naegele's Rule with cycle-length adjustment. Supports irregular cycles (20-45 days).

### 🌐 [Pregnancy Week Calculator](https://ganak.app/calculators/pregnancy-week-calculator/)

Free pregnancy week calculator to find how many weeks pregnant you are. Track your baby's development week by week with size comparisons, symptoms, and milestones. Calculate by LMP, conception date, IVF transfer, ultrasound, or due date. Includes trimester timeline, weeks-to-months conversion, and 42-week development guide. Supports irregular cycles (20-45 days).

### 🌐 [QTc Calculator](https://ganak.app/calculators/qtc-calculator/)

Free QTc calculator using Bazett, Fridericia, Framingham, Hodges & Rautaharju formulas. Enter heart rate and QT interval for instant corrected QT with gender-specific normal ranges, visual risk scale, and side-by-side formula comparison.

### 🌐 [Sleep Calculator](https://ganak.app/calculators/sleep-calculator/)

Calculate optimal bedtime and wake-up times based on 90-minute sleep cycles. Find your ideal sleep schedule by age group with a sleep quality score, nap recommendations, and health impact analysis. Uses National Sleep Foundation guidelines with adjustable fall-asleep time. Includes sleep cycle stages, sleep duration by age chart, and sleep hygiene tips.

### 🌐 [TDEE Calculator](https://ganak.app/calculators/tdee-calculator/)

Calculate your Total Daily Energy Expenditure (TDEE) - the total calories you burn per day including all activities. Essential for weight loss (calorie deficit), weight gain (surplus), or maintenance. Uses BMR plus activity multipliers for sedentary, light, moderate, active, and very active lifestyles. Includes macronutrient recommendations.

### 🌐 [Waist to Height Ratio Calculator](https://ganak.app/calculators/waist-to-height-ratio-calculator/)

Calculate your waist-to-height ratio (WHtR) instantly and check your health risk. WHtR is a better predictor of cardiometabolic risk than BMI. Get your Ashwell shape chart category, ideal waist target, body fat estimate, and personalized health assessment. Supports inches and centimeters.

### 🌐 [Water Intake Calculator](https://ganak.app/calculators/water-intake-calculator/)

Calculate your daily water intake based on weight, activity level, climate, and exercise. Get a personalized hydration schedule with glasses, liters, and ounces. Includes hourly drinking plan, adjustment factors for caffeine and exercise, and evidence-based recommendations from IOM guidelines.

[⬆ Back to contents](#contents)

## 🏋️ [Fitness](https://ganak.app/calculators/category/fitness/)

### 🌐 [One Rep Max Calculator](https://ganak.app/calculators/one-rep-max-calculator/)

Estimate your One Rep Max (1RM) for any exercise using proven strength training formulas (Epley, Brzycki, Lombardi). Calculate maximum weight you can lift for one repetition based on submaximal lifts. Essential for powerlifting, bodybuilding, strength programming, and progressive overload training. Includes percentage charts for training zones.

### 🌐 [Pace Calculator](https://ganak.app/calculators/pace-calculator/)

Free running pace calculator with race splits, finish time predictions, and pace conversion. Calculate pace for 5K, 10K, half marathon, marathon and any distance. Get per-mile and per-km splits instantly with treadmill speed equivalents.

### 🌐 [Protein Intake Calculator](https://ganak.app/calculators/protein-intake-calculator/)

Calculate your optimal daily protein intake based on body weight, activity level, and fitness goals. Get personalized recommendations for muscle building, fat loss, maintenance, or body recomposition with per-meal distribution and food sources.

### 🌐 [VO2 Max Calculator](https://ganak.app/calculators/vo2-max-calculator/)

Calculate your VO2 Max (maximal oxygen uptake) from multiple test methods: Cooper 12-minute run, 1.5-mile run, Rockport walk test, heart rate, or race results (VDOT). Get fitness classification by age and sex, predicted race times, and personalized training insights based on ACSM guidelines.

[⬆ Back to contents](#contents)

## 📅 [Date & Time](https://ganak.app/calculators/category/date-and-time/)

### 🌐 [Add/Subtract Date Calculator](https://ganak.app/calculators/date-calculator/date-add/)

Add or subtract years, months, weeks, or days from any starting date. Calculate future deadlines, expiration dates, delivery schedules, and recurring events. Essential for contract end dates, subscription renewals, project milestones, and financial maturity dates. Handles month-end edge cases correctly.

### 🌐 [Age Calculator](https://ganak.app/calculators/age-calculator/)

Calculate your exact age in years, months, weeks, and days from your birth date. Find total days lived, upcoming birthday countdown, zodiac sign, and generation. Perfect for birthday celebrations, age verification, retirement planning, and milestone tracking. Supports past and future date calculations.

### 🌐 [Birthday Calculator](https://ganak.app/calculators/birthday-calculator/)

Calculate your exact age in years, months, and days with birthday countdown, zodiac sign, birth stone, Chinese zodiac, golden birthday, half birthday, and life path number. Free birthday calculator with instant results — all calculations run in your browser.

### 🌐 [Business Days Calculator](https://ganak.app/calculators/date-calculator/business-days/)

Add a specific number of business days to any date, automatically skipping weekends and holidays. Calculate delivery dates, payment due dates, project deadlines, and service level agreements. Essential for shipping estimates, legal filings, HR processes, and business operations planning.

### 🌐 [Date Calculator](https://ganak.app/calculators/date-calculator/)

All-in-one date calculator for finding differences between dates, adding or subtracting days/weeks/months/years, and counting business days. Features duration calculator, workday counter, date arithmetic, and week number finder. Essential for project planning, deadline tracking, event scheduling, and contract calculations.

### 🌐 [Date Duration Calculator](https://ganak.app/calculators/date-calculator/duration/)

Calculate the exact duration between two dates with precision down to hours and minutes. Find years, months, weeks, days, hours, and total time elapsed. Perfect for age calculations, project timelines, event countdowns, pregnancy due dates, and anniversary tracking. Supports timezone-aware calculations.

### 🌐 [Days Calculator](https://ganak.app/calculators/days-calculator/)

Calculate the number of days between two dates or add/subtract days from any date. Shows total days, weeks, months, hours, minutes, weekdays, and weekend days. Supports inclusive and exclusive counting. Free online days calculator with instant results for project planning, deadlines, countdowns, and date arithmetic.

### 🌐 [Hours Calculator](https://ganak.app/calculators/hours-calculator/)

Calculate hours worked between any two times, with optional break deduction. Switch to Weekly Timecard mode to track daily shifts across 7 days with totals. Add your hourly rate to calculate gross pay and overtime. Ideal for employees, freelancers, nurses, and anyone tracking billable or payroll hours. Overnight shifts detected automatically.

### 🌐 [Week Number Calculator](https://ganak.app/calculators/date-calculator/week-number/)

Get the ISO 8601 week number for any date, following international standard week numbering. Find which week of the year a date falls in, useful for weekly reporting, payroll processing, project tracking, and manufacturing schedules. Includes year-week format (e.g., 2024-W15).

### 🌐 [Weekday Finder](https://ganak.app/calculators/date-calculator/weekday/)

Find the day of the week (Monday-Sunday) for any date in history or future. Also shows day-of-year number (1-365/366), week number, and whether it falls on a weekend. Perfect for historical research, event planning, scheduling, and answering 'What day was I born?' questions.

### 🌐 [Workdays Between Dates Calculator](https://ganak.app/calculators/date-calculator/workdays/)

Count business days (working days) between two dates, excluding weekends and holidays. Calculate project timelines, SLA deadlines, delivery estimates, and payment terms. Essential for HR leave calculations, legal deadlines, shipping estimates, and business contract compliance. Supports custom holiday calendars.

[⬆ Back to contents](#contents)

## 🏗️ [Construction](https://ganak.app/calculators/category/construction/)

### 🌐 [Asphalt Calculator](https://ganak.app/calculators/asphalt-calculator/)

Estimate asphalt tonnage, volume, and cost for driveways, roads, and parking lots. Supports hot mix, warm mix, cold mix, and porous asphalt with imperial and metric units.

### 🌐 [Brick Calculator](https://ganak.app/calculators/brick-calculator/)

Free brick calculator. Enter wall dimensions to get exact brick count, mortar bags, sand, weight, and cost. Supports 8 brick sizes, openings, waste factor, and imperial/metric units.

### 🌐 [Concrete Calculator](https://ganak.app/calculators/concrete-calculator/)

Estimate concrete volume in yards, feet, or meters for slabs, footings, walls, and columns. Includes bag count and cost estimates.

### 🌐 [Fence Calculator](https://ganak.app/calculators/fence-calculator/)

Estimate fence materials, posts, pickets, and total cost. Compare wood, vinyl, chain link, and metal fencing with instant DIY or professional cost breakdown.

### 🌐 [Gravel Calculator](https://ganak.app/calculators/gravel-calculator/)

Estimate gravel tonnage, volume in yards, and cost for driveways, landscaping, and drainage projects. Supports pea gravel, crushed stone, river rock, limestone, and more with imperial and metric units.

### 🌐 [Mulch Calculator](https://ganak.app/calculators/mulch-calculator/)

Estimate mulch volume in cubic yards, cubic feet, or meters for gardens, landscaping, and playgrounds. Includes bag count, weight, and cost estimates for hardwood, cedar, pine bark, rubber, and more.

### 🌐 [Multi-Room Square Footage Calculator](https://ganak.app/calculators/multiroom-square-footage-calculator/)

Calculate total square footage for multiple rooms at once. Add rooms with quantities for large projects, apartments, or commercial spaces. Get combined area totals with material cost estimation.

### 🌐 [Paint Calculator](https://ganak.app/calculators/paint-calculator/)

Free paint calculator. Enter room or house dimensions to get exact gallons needed, quart breakdown, primer estimate, and cost — for interior and exterior surfaces.

### 🌐 [Rebar Calculator](https://ganak.app/calculators/rebar-calculator/)

Free rebar calculator. Estimate rebar quantity, spacing, weight, and cost for concrete slabs, footings, and walls. Instant results with bar size reference.

### 🌐 [Room Square Footage Calculator](https://ganak.app/calculators/room-square-footage-calculator/)

Calculate room or floor area in square feet. Perfect for flooring, carpet, tile, and paint projects. Simply enter length and width in feet and inches to get instant area calculations with unit conversions.

### 🌐 [Septic Tank Size Calculator](https://ganak.app/calculators/septic-tank-size-calculator/)

Free septic tank size calculator. Enter bedrooms or occupants to get recommended tank capacity, dimensions, pumping schedule, and drainfield size. Imperial & metric.

### 🌐 [Shapes Area Calculator](https://ganak.app/calculators/shapes-area-calculator/)

Calculate irregular and complex area shapes using rectangles, rectangle borders, circles, rings, triangles (base/height or sides), trapezoids, sectors, and parallelograms. Perfect for roofing, landscaping, decks, and custom construction projects.

### 🌐 [Soil Calculator](https://ganak.app/calculators/soil-calculator/)

Estimate soil volume in cubic yards, cubic feet, or meters for gardens, raised beds, and landscaping. Includes bag count, weight, and cost estimates for topsoil, compost, potting mix, and more.

### 🌐 [Square Footage Calculator](https://ganak.app/calculators/square-footage-calculator/)

Free online square footage calculator for rooms, walls, and irregular shapes. Calculate area in sq ft, sq m, sq yd, and acres. Includes feet & inches input, multi-room totals, waste factor for materials, and cost estimation. Perfect for flooring, tile, paint, roofing, and construction projects.

### 🌐 [Stair Calculator](https://ganak.app/calculators/stair-calculator/)

Calculate stair dimensions instantly. Get riser height, tread depth, stringer length, stair angle, and building code compliance with an interactive diagram.

### 🌐 [Wall Area Calculator](https://ganak.app/calculators/wall-area-calculator/)

Calculate wall area for painting or wallpaper, automatically subtracting doors and windows. Get accurate material estimates with waste factor for paint coverage and wallpaper rolls.

[⬆ Back to contents](#contents)

## 🔄 [Converters](https://ganak.app/calculators/category/converters/)

### 🌐 [Astronomy Distance Converter](https://ganak.app/calculators/astronomy-length-converter/)

Astronomy distance converter for cosmic measurements. Convert between light years, parsecs, astronomical units (AU), and kilometers. Essential for understanding stellar distances, galaxy sizes, and space exploration. Perfect for astronomy enthusiasts, students, and researchers studying the universe.

### 🌐 [Body Weight Converter](https://ganak.app/calculators/body-weight-converter/)

Body weight converter for fitness and health tracking. Convert between kilograms (kg), pounds (lbs), and stone. Perfect for weight loss tracking, gym progress monitoring, and international weight comparisons. Commonly used ranges optimized for human body weight measurements.

### 🌐 [Cooking Weight Converter](https://ganak.app/calculators/cooking-weight-converter/)

Kitchen weight converter for recipe measurements. Convert between grams, ounces, pounds, cups, tablespoons, and teaspoons. Perfect for scaling recipes, converting international cookbooks, and precise baking measurements. Optimized presets for common cooking ingredients and portion sizes.

### 🌐 [Currency Converter](https://ganak.app/calculators/currency-converter/)

Convert between 150+ world currencies with daily updated mid-market exchange rates. Free currency converter with 90-day trend charts, quick reference tables, and instant cross-rate calculations. Rates sourced from Floatrates and the European Central Bank.

### 🌐 [Gold & Jewelry Weight Converter](https://ganak.app/calculators/gold-jewelry-weight-converter/)

Gold and jewelry weight converter supporting Tola, Grams, Carats, Troy Ounces, Grains, and Pennyweights. Essential for gold traders, jewelers, and buyers in India, UAE, and worldwide. Accurate conversions for calculating gold prices, jewelry valuations, and precious metal transactions.

### 🌐 [GSM Calculator](https://ganak.app/calculators/gsm-calculator/)

Free GSM calculator for fabric, paper and materials. Calculate grams per square meter from sample weight, find material weight from GSM, or compute roll weight. Converts GSM to oz/yd², lbs/ream and g/ft² with instant results and a material reference guide covering paper, fabric, towels and packaging.

### 🌐 [Length Converter Calculator](https://ganak.app/calculators/length-converter/)

Free online length converter supporting 20+ units: centimeters (cm), meters (m), kilometers (km), feet (ft), inches (in), miles, yards, and more. Instant bidirectional conversion with high precision. Ideal for real estate measurements, travel distances, construction projects, scientific calculations, and international unit conversions.

### 🌐 [Real Estate Length Converter](https://ganak.app/calculators/real-estate-length-converter/)

Real estate length converter for property measurements in India and worldwide. Convert between feet, meters, gaj, guz, yards, and square units. Essential for land measurement, property area calculations, plot dimensions, and real estate transactions. Includes traditional Indian units used in property dealings.

### 🌐 [Scientific Length Converter](https://ganak.app/calculators/scientific-length-converter/)

Scientific length converter for research and laboratory measurements. Convert between nanometers (nm), micrometers (μm), angstroms (Å), picometers, and femtometers. Essential for nanotechnology, molecular biology, physics research, and semiconductor manufacturing. High-precision conversions for scientific accuracy.

### 🌐 [Scientific Weight Converter](https://ganak.app/calculators/scientific-weight-converter/)

Scientific weight converter for laboratory measurements. Convert between micrograms (μg), milligrams (mg), grams (g), atomic mass units (AMU), and moles. Essential for chemistry, pharmacology, biochemistry research, and precision scientific experiments. High-precision conversions for laboratory accuracy.

### 🌐 [Shipping Weight Converter](https://ganak.app/calculators/shipping-weight-converter/)

Shipping and logistics weight converter. Convert between kilograms, pounds, tonnes, metric tons, and hundredweights. Essential for freight calculations, cargo shipping, warehouse management, and international trade. Optimized for commercial shipping weight ranges and packaging calculations.

### 🌐 [Shoe Size Converter](https://ganak.app/calculators/shoe-size-converter/)

Free shoe size converter supporting US, UK, EU, JP, CM, MX, BR, and KR sizing systems. Instantly convert men's, women's, and kids' shoe sizes with cross-gender equivalents and foot length measurements. Based on empirical lookup tables cross-referenced against Brannock device standards, ISO 9407, and major brand charts. BR sizes use a US + 2 approximation.

### 🌐 [Square Footage Converter](https://ganak.app/calculators/square-footage-converter/)

Convert between square feet, square meters, square yards, acres, hectares, and more. Quick and accurate area unit conversion with all common measurement units.

### 🌐 [Temperature Converter Calculator](https://ganak.app/calculators/temperature-converter/)

Free online temperature converter supporting Celsius (°C), Fahrenheit (°F), Kelvin (K), and Rankine (°R). Instant bidirectional conversion with formulas displayed. Essential for cooking, weather, scientific experiments, HVAC calculations, and international temperature comparisons. Includes common reference points like freezing and boiling points.

### 🌐 [Time to Decimal Calculator](https://ganak.app/calculators/time-to-decimal-calculator/)

Free time to decimal converter for payroll, billing, and time tracking. Convert hours, minutes, and seconds to decimal hours instantly. Bidirectional conversion with payroll calculator, precision control, and a complete minutes-to-decimal reference table. FLSA compliant.

### 🌐 [Travel Distance Converter](https://ganak.app/calculators/travel-distance-converter/)

Travel distance converter for road trips, flights, and maritime navigation. Convert between kilometers, miles, nautical miles, and leagues. Perfect for trip planning, fuel calculations, flight distances, and international travel. Includes commonly used distance ranges for cars, planes, and ships.

### 🌐 [Weight Converter Calculator](https://ganak.app/calculators/weight-converter/)

Free online weight converter supporting 15+ units: kilograms (kg), grams (g), pounds (lbs), ounces (oz), stone, milligrams, tonnes, and more. Instant bidirectional conversion with precision up to 6 decimal places. Perfect for cooking recipes, shipping calculations, fitness tracking, scientific measurements, and international unit conversions.

### 🇮🇳 [Hectare to Bigha Converter](https://ganak.app/calculators/india/hectare-to-bigha-converter/)

Free hectare to bigha converter with state-wise conversion rates for all Indian states including UP, Rajasthan, Bihar, Gujarat, MP, West Bengal, Assam, Himachal Pradesh, and more. Instantly convert between hectares and bigha with accurate regional rates, reverse conversion, equivalent areas in sq m, sq ft, and acres, and a full state comparison table.

### 🇮🇳 [Indian Land Unit Converter](https://ganak.app/calculators/india/indian-land-unit-converter/)

Free Indian land unit converter with 20+ units including Bigha, Gaj, Guntha, Cent, Katha, Marla, Kanal, Biswa, Dismil, Acre, Hectare, Square Feet, and Square Metres. State-wise conversion rates for variable units across 22 Indian states. Instant bidirectional conversion with all-unit equivalents, quick reference tables, and shareable URLs.

[⬆ Back to contents](#contents)

## 🎓 [Academic](https://ganak.app/calculators/category/academic/)

### 🌐 [Attendance Calculator](https://ganak.app/calculators/attendance-calculator/)

Free attendance calculator for school, college, university, and office. Find your current attendance percentage, how many classes or days you need to attend to reach 75% (or any target), and how many you can safely miss. Track by classes or days — works for students, employees, and any institution with minimum attendance requirements.

### 🌐 [CGPA Calculator](https://ganak.app/calculators/cgpa-calculator/)

Free CGPA calculator to compute your Cumulative Grade Point Average from semester-wise SGPAs and credits. Supports 10-point (Indian universities), 4.0 (US/UK), and 5.0 grading scales. Instantly converts CGPA to percentage, shows semester-by-semester breakdown with running CGPA, SGPA trend analysis, and grade classification. Essential for university students, job applications, and graduate admissions.

### 🌐 [CGPA to Percentage Calculator](https://ganak.app/calculators/cgpa-to-percentage-calculator/)

Convert CGPA to percentage using standard grading scales (10-point, 4.0 GPA, 5-point). Supports multiple conversion factors including 9.5 (AICTE India), 9.0, and custom multipliers. Essential for university applications, job applications, and academic transcript conversions. Formula: Percentage = CGPA × Grading Scale factor.

### 🌐 [GPA Calculator](https://ganak.app/calculators/gpa-calculator/)

Free GPA calculator for college, high school, and university students. Calculate semester and cumulative GPA on a 4.0 scale with letter grades and credit hours. Plan your target GPA with the built-in GPA planner. Supports weighted GPA for Honors, AP, and IB courses, plus Pass/Fail grading.

### 🌐 [Grade Calculator](https://ganak.app/calculators/grade-calculator/)

Calculate your weighted grade average, find out what you need on your final exam, or convert test scores to letter grades. Supports weighted assignments, final grade planning, and test grading with US letter grades (A+ to F) and GPA equivalents.

### 🌐 [Negative Marking Calculator](https://ganak.app/calculators/negative-marking-calculator/)

Calculate exam scores with negative marking for competitive exams like GATE, JEE, NEET, UPSC, and banking tests. Instantly compute net score, accuracy percentage, and penalty deductions. Supports custom marking schemes with variable positive and negative marks per question. Essential for exam preparation strategy and score prediction.

### 🌐 [Percentage to CGPA Calculator](https://ganak.app/calculators/percentage-to-cgpa-calculator/)

Convert percentage to CGPA using standard grading scales (10-point, 4.0 GPA, 5-point). Reverse CGPA calculation for academic equivalency, study abroad applications, and GPA conversions. Supports custom conversion factors for different universities and grading systems worldwide. Formula: CGPA = Percentage ÷ Grading Scale factor.

### 🌐 [SAT Score Calculator](https://ganak.app/calculators/sat-score-calculator/)

Convert your Digital SAT raw scores to estimated scaled scores (200-800 per section, 400-1600 total). Enter correct answers for Reading & Writing (54 questions) and Math (44 questions) to see your predicted score, percentile ranking, and college competitiveness level. Based on College Board equating tables.

### 🌐 [SGPA to CGPA Calculator](https://ganak.app/calculators/sgpa-to-cgpa-calculator/)

Free SGPA to CGPA calculator to convert your semester-wise SGPAs to cumulative CGPA. Supports simple average and credit-weighted calculation methods across 10-point (Indian universities like VTU, KTU, BPUT, Mumbai University), 4.0 (US/UK), and 5.0 grading scales. Instantly shows CGPA, percentage equivalent, semester breakdown, SGPA trend chart, and grade classification.

[⬆ Back to contents](#contents)

## 🎨 [Design](https://ganak.app/calculators/category/design/)

### 🌐 [Aspect Ratio Calculator](https://ganak.app/calculators/aspect-ratio-calculator/)

Free aspect ratio calculator. Enter width & height to get the simplified ratio (16:9, 4:3, 1:1). Scale dimensions proportionally, or pick from 40+ presets for Instagram, YouTube, LinkedIn, TikTok, and more.

### 🌐 [DPI Converter](https://ganak.app/calculators/dpi-converter/)

Free DPI converter. Convert pixels to print size in inches and cm at any DPI (72, 200, 300, 600). Calculate required pixels for a print job, convert mouse DPI sensitivity, and use the DPI reference table for web, government forms, and professional printing.

### 🌐 [PX to REM Converter](https://ganak.app/calculators/px-to-rem-converter/)

Free PX to REM converter. Convert pixels to rem (and rem to px) with any base font size. Get equivalent values in em, percent, and points. Paste CSS code to bulk-convert all px values to rem instantly.

[⬆ Back to contents](#contents)

## 💻 [Tech](https://ganak.app/calculators/category/tech/)

### 🌐 [PC Bottleneck Calculator](https://ganak.app/calculators/bottleneck-calculator/)

Free PC bottleneck calculator. Enter your CPU, GPU & RAM specs to instantly check system balance, identify bottlenecks, and get personalized upgrade recommendations for gaming, streaming, video editing, and productivity workloads.

### 🌐 [PC PSU Calculator](https://ganak.app/calculators/psu-calculator/)

Calculate the right PSU wattage for your PC build. Enter CPU and GPU power draw, storage, fans, and cooling to get recommended power supply size, per-component breakdown, efficiency tier comparison, and annual electricity cost estimates. Vendor-neutral, no product bias.

### 🌐 [PPI Calculator](https://ganak.app/calculators/ppi-calculator/)

Calculate the pixel density (PPI) of any display. Enter your screen's resolution and diagonal size, or pick from 30+ popular phones, tablets, laptops, monitors, and TVs. Instantly see PPI, dot pitch, megapixels, physical dimensions, and whether your display is Retina-quality at your viewing distance.

### 🌐 [RAID Calculator](https://ganak.app/calculators/raid-calculator/)

Calculate RAID array capacity, storage efficiency, fault tolerance, and performance for RAID 0, 1, 5, 6, 10, 50, and 60. Compare RAID levels side-by-side with cost-per-TB analysis. Supports hot spares, filesystem overhead, and drive type speed estimates.

### 🌐 [Screen Size Calculator](https://ganak.app/calculators/screen-size-calculator/)

Calculate the physical width, height, and area of any screen from its diagonal size and aspect ratio. Add resolution for PPI, dot pitch, and megapixel count. Auto-detect your screen resolution, get SMPTE and THX viewing distance recommendations, and check Retina quality. Supports all aspect ratios including 16:9, 21:9, 4:3, and custom.

### 🌐 [Subnet Calculator](https://ganak.app/calculators/subnet-calculator/)

Calculate subnet details for any IPv4 or IPv6 address. Enter an IP and CIDR prefix to instantly get network address, broadcast address, usable host range, wildcard mask, and total hosts. Includes full CIDR reference table and binary breakdown.

[⬆ Back to contents](#contents)

## 🚗 [Auto](https://ganak.app/calculators/category/auto/)

### 🌐 [Gear Ratio Calculator](https://ganak.app/calculators/gear-ratio-calculator/)

Free gear ratio calculator. Find gear ratio from tooth counts, calculate RPM and output speed, compute vehicle speed from drivetrain specs, and analyze bicycle gearing with gear inches and development.

### 🌐 [RPM Calculator](https://ganak.app/calculators/rpm-calculator/)

Free RPM calculator. Calculate engine RPM from speed, tire size, and gear ratio. Includes pulley RPM, SFM to RPM for machining, and torque-to-horsepower conversion. Instant results with charts.

### 🌐 [Tire Size Calculator](https://ganak.app/calculators/tire-size-calculator/)

Free tire size calculator. Compare two tire sizes side by side — see diameter, sidewall, circumference, speedometer error, and ground clearance difference. Instant results with safety indicator.

### 🌐 [Towing Capacity Calculator](https://ganak.app/calculators/towing-capacity-calculator/)

Free towing capacity calculator. Check payload, towing capacity, GCVW, tongue weight, and hitch limits against your vehicle's GVWR and GCWR. Instant safety status with visual utilization bars.

[⬆ Back to contents](#contents)

## ✈️ [Travel](https://ganak.app/calculators/category/travel/)

### 🌐 [Fuel Cost Calculator](https://ganak.app/calculators/fuel-cost-calculator/)

Calculate fuel costs for any trip. Enter distance, fuel efficiency, and gas price to estimate travel expenses, cost per mile, and monthly fuel budget.

### 🌐 [Toll Calculator](https://ganak.app/calculators/toll-calculator/)

Calculate toll costs for any trip. Compare pass vs cash pricing, estimate commuter costs, and plan your travel budget with vehicle-specific toll rates.

[⬆ Back to contents](#contents)

## 🎲 [Fun](https://ganak.app/calculators/category/fun/)

### 🌐 [Dog Age Calculator](https://ganak.app/calculators/dog-age-calculator/)

Calculate your dog's age in human years using the modern AKC size-based method. Enter your dog's age and size (small, medium, large, or giant) to get their human-equivalent age, life stage, health tips, and a full comparison chart. Free, instant, and accurate — the old 7× rule is a myth.

### 🌐 [FLAMES Calculator](https://ganak.app/calculators/flames-calculator/)

Play the classic FLAMES game online. Enter two names to find if you are Friends, Lovers, Affection, Marriage, Enemies, or Siblings. See step-by-step letter elimination, compatibility percentage, and detailed name analysis. Free, instant, and private — all calculations run in your browser.

### 🌐 [Love Calculator](https://ganak.app/calculators/love-calculator/)

Calculate your love compatibility by name, FLAMES game, or numerology. Get your love percentage with detailed compatibility scores for Name Harmony, Letter Chemistry, and Sound Compatibility. Play the classic FLAMES game or use birth dates for numerology-based love matching. Free, instant, and private — all calculations run in your browser.

[⬆ Back to contents](#contents)

## 🔮 [Spirituality](https://ganak.app/calculators/category/spirituality/)

### 🌐 [Angel Number Calculator](https://ganak.app/calculators/angel-number-calculator/)

Free angel number calculator. Calculate your personal angel number from your date of birth and name. Discover your angel number meaning, spiritual guidance, and divine message. Supports birthday, name, and combined methods with step-by-step calculation breakdown. Find master numbers 11, 22, 33 and learn about repeating sequences like 111, 222, 333.

### 🌐 [Birth Chart Calculator](https://ganak.app/calculators/birth-chart-calculator/)

Free birth chart calculator. Get your Sun, Moon, and Rising signs instantly with accurate planetary positions, house placements, aspects, and element balance. Supports both Tropical (Western) and Sidereal (Vedic) zodiac systems. Powered by NASA JPL ephemeris data.

### 🌐 [Darakaraka Calculator](https://ganak.app/calculators/darakaraka-calculator/)

Free Darakaraka calculator. Find your Darakaraka (spouse indicator) and all 7 Jaimini Chara Karakas from your birth chart. Discover the nature of your life partner based on planetary degrees. Supports 7-planet and 8-planet (with Rahu) karaka schemes. Powered by NASA JPL ephemeris data with Sidereal and Tropical zodiac systems.

### 🌐 [Dasha Calculator](https://ganak.app/calculators/dasha-calculator/)

Free Vimshottari Dasha calculator. Find your current Mahadasha, Antardasha, and Pratyantardasha periods from your date of birth. See your complete 120-year planetary timeline with dates, progress indicators, and period interpretations. Powered by NASA JPL ephemeris data.

### 🌐 [Gematria Calculator](https://ganak.app/calculators/gematria-calculator/)

Free gematria calculator. Instantly compute Hebrew, English, Jewish, and Greek gematria values across 6+ cipher systems including English Ordinal, Full Reduction, Reverse Ordinal, and more. Enter any word or phrase for a complete letter-by-letter breakdown.

### 🌐 [Lo Shu Grid Calculator](https://ganak.app/calculators/lo-shu-grid-calculator/)

Free Lo Shu Grid calculator. Enter your date of birth to generate your personal Lo Shu Grid with plane analysis, present and absent arrows, missing and repeated number interpretations, Driver, Conductor, and Kua numbers. Instant results with detailed personality insights based on Chinese numerology.

### 🌐 [Moon Sign Calculator](https://ganak.app/calculators/moon-sign-calculator/)

Free moon sign calculator. Find your lunar zodiac sign based on birth date, time, and location. Discover your emotional nature, inner self, and moon phase at birth. Supports Tropical and Sidereal zodiac systems. Works without birth time.

### 🌐 [Nakshatra Calculator](https://ganak.app/calculators/nakshatra-calculator/)

Free nakshatra calculator. Find your birth star (nakshatra), pada, rashi, and nakshatra lord from your date of birth. Discover your Vedic lunar mansion with detailed personality traits, deity, ganam, nadi, and yoni. Powered by NASA JPL ephemeris data.

### 🌐 [Name Numerology Calculator](https://ganak.app/calculators/name-numerology-calculator/)

Free name numerology calculator. Instantly calculate your Expression, Soul Urge, and Personality numbers using Pythagorean or Chaldean methods. Analyze personal or business names with letter-by-letter breakdown, number frequency chart, missing number analysis, and side-by-side system comparison. Try alternate spellings to find your luckiest name.

### 🌐 [Numerology Calculator](https://ganak.app/calculators/numerology-calculator/)

Free numerology calculator. Get your Life Path, Expression, Soul Urge, Personality, Birthday, and Personal Year numbers using Pythagorean or Chaldean methods. Enter your name and date of birth for instant results with step-by-step calculation breakdown and detailed number meanings.

### 🌐 [Rising Sign Calculator](https://ganak.app/calculators/rising-sign-calculator/)

Free rising sign calculator. Find your ascendant zodiac sign based on birth date, time, and location. Discover your outward personality, chart ruler, and how others perceive you. Supports Tropical and Sidereal zodiac systems. Shows all possible rising signs if birth time is unknown.

### 🌐 [Sade Sati Calculator](https://ganak.app/calculators/sade-sati-calculator/)

Free Sade Sati calculator. Find your Shani Sade Sati periods, current phase, and exact dates from your date of birth. Instant results with phase breakdown, Ashtama Sani check, Kandaka Sani detection, and Moon sign analysis. Powered by NASA JPL ephemeris data.

### 🌐 [Saturn Return Calculator](https://ganak.app/calculators/saturn-return-calculator/)

Free Saturn Return calculator. Find your exact Saturn Return dates with multi-pass retrograde detail, influence windows, and personalized life themes. Powered by NASA JPL ephemeris data. Supports Tropical and Sidereal zodiac systems. See if you are currently in your Saturn Return.

### 🌐 [Tithe Calculator](https://ganak.app/calculators/tithe-calculator/)

Free tithe calculator. Calculate your tithe from salary on gross or net income. Enter annual, monthly, bi-weekly, weekly, or hourly income and instantly see your 10% tithe broken down by period. Includes additional offering tracker, remaining income breakdown, and support for 6 currencies. Answers: how to calculate tithe from salary, gross vs net tithe, and tithing for business owners.

### 🌐 [Vastu Calculator](https://ganak.app/calculators/vastu-calculator/)

Free Vastu calculator with instant score, Ayadi Shadvarga formulas, room size checker & Brahmasthan finder. Check your home or flat vastu online — no signup needed.

### 🌐 [Zakat Calculator](https://ganak.app/calculators/zakat-calculator/)

Free Zakat calculator with customizable Nisab rates. Calculate Zakat on gold, silver, cash, investments and business assets. Supports 8 currencies, gold/silver Nisab standards, and madhab-aware debt handling (Shafi'i excludes debts). Per-category breakdown with shareable results.

[⬆ Back to contents](#contents)

## 🎯 [Probability](https://ganak.app/calculators/category/probability/)

### 🌐 [Blackjack Calculator](https://ganak.app/calculators/blackjack-calculator/)

Free blackjack calculator with 3 modes: get optimal strategy advice for any hand, view the full basic strategy chart, and calculate house edge for your game rules. Shows expected value for every action.

### 🌐 [Odds Calculator](https://ganak.app/calculators/odds-calculator/)

Free odds calculator with 5 modes: convert between American, decimal & fractional odds, calculate bet payouts and profit, find implied probability, remove bookmaker vig for fair odds, and compute parlay payouts.

[⬆ Back to contents](#contents)

## 👗 [Styling](https://ganak.app/calculators/category/styling/)

### 🌐 [Body Shape Calculator](https://ganak.app/calculators/body-shape-calculator/)

Determine your body shape based on bust, waist, and hip measurements. Get personalized style recommendations for your unique body shape.

### 🌐 [Bra Size Calculator](https://ganak.app/calculators/bra-size-calculator/)

Free bra size calculator. Enter underbust and bust measurements to find your bra size in US, UK, EU, AU & JP systems. Includes sister sizes, international size chart, and step-by-step measurement guide.

### 🌐 [Clothing Size Converter](https://ganak.app/calculators/clothing-size-converter/)

Free clothing size converter for women's, men's, and kids' sizes. Convert between US, UK, EU, AU, IT, FR, JP sizing systems instantly. Includes body measurement mode, cross-gender conversion, and complete international size charts.

### 🌐 [Dress Size Calculator](https://ganak.app/calculators/dress-size-calculator/)

Free dress size calculator. Enter bust, waist, and hip measurements to find your size in US, UK, EU, AU & IT systems. Includes plus sizes, height & weight estimation mode, and international size chart.

### 🌐 [Ring Size Calculator](https://ganak.app/calculators/ring-size-calculator/)

Free ring size calculator supporting US, UK, EU, Indian, and Japanese sizing systems. Convert between ring sizes using circumference or diameter measurements in mm or inches. Includes a complete international ring size chart with cross-system conversion and step-by-step measurement guide.

### 🌐 [Shirt Size Calculator](https://ganak.app/calculators/shirt-size-calculator/)

Free shirt size calculator for men and women. Enter neck, chest, and sleeve measurements or use height and weight to find your dress shirt size, t-shirt size, and international equivalents in US, UK, and EU systems. Includes fit preference, between-size guidance, and complete size charts.

### 🌐 [Suit Size Calculator](https://ganak.app/calculators/suit-size-calculator/)

Free suit size calculator. Enter chest and waist measurements or use height and weight to find your suit jacket and pants size in US, UK, EU, and Italian systems. Includes jacket length, between-size guidance, sleeve reference for alterations, and complete international size charts.

### 🌐 [Tablecloth Size Calculator](https://ganak.app/calculators/tablecloth-size-calculator/)

Free tablecloth size calculator. Enter table dimensions and drop style to find the ideal tablecloth size for rectangle, round, oval, and square tables in inches or centimeters.

[⬆ Back to contents](#contents)

## 🏛️ [Government Schemes](https://ganak.app/calculators/category/government-schemes/)

### 🇮🇳 [NPS Calculator](https://ganak.app/calculators/india/nps-calculator/)

Calculate NPS (National Pension System) retirement corpus, monthly pension, and lump sum withdrawal. Estimate returns with step-up SIP, employer contribution, and tax savings under Section 80CCD(1B). Compare NPS pension fund managers. Free NPS pension calculator for salaried and self-employed Indians.

### 🇮🇳 [PPF Calculator](https://ganak.app/calculators/india/ppf-calculator/)

Calculate PPF (Public Provident Fund) maturity amount, yearly interest, and total returns over 15 years in India. Estimate EEE tax benefits under Section 80C (up to ₹1.5 lakh deduction), plan annual contributions, and compare PPF with other safe investments. PPF interest rate: 7.1% p.a. as of Q4 FY 2025-26, compounded yearly. Government-backed retirement savings.

### 🇮🇳 [Sukanya Samriddhi Yojana Calculator](https://ganak.app/calculators/india/sukanya-samriddhi-yojana-calculator/)

Calculate Sukanya Samriddhi Yojana (SSY) maturity amount for girl child savings in India. Estimate total returns at age 21, plan yearly deposits (₹250 to ₹1.5 lakh), and understand partial withdrawal rules at age 18. Current SSY interest rate: 8.2% p.a. Among the highest rates among small savings schemes with Section 80C tax benefits. Essential for daughter's education and marriage planning.

[⬆ Back to contents](#contents)

## 📦 [Others](https://ganak.app/calculators/category/others/)

### 🌐 [Discount Calculator](https://ganak.app/calculators/discount-calculator/)

Free discount calculator with tax (GST, VAT, sales tax). Calculate percentage off, flat discounts, double/stacked discounts (20% + 10%), and find the original price from a sale price. Compare discount deals across 9 currencies. Essential for shopping sales, coupons, Black Friday deals, and price negotiations.

### 🌐 [Rent Calculator](https://ganak.app/calculators/rent-calculator/)

Free rent affordability calculator. Enter your income (annual salary, monthly, bi-weekly, or hourly wage), existing debts, savings goals, and utility costs to find how much rent you can afford. Compare the 30% rule, 28/36 rule, 50/30/20 rule, or set a custom percentage. Includes move-in cost estimator and budget breakdown chart. Supports 11 currencies.

### 🌐 [Snow Day Calculator](https://ganak.app/calculators/snow-day-calculator/)

Get your snow day probability using real live weather data from Open-Meteo. Search for any city worldwide, pick a forecast day, and our transparent scoring formula instantly shows how snowfall, temperature, wind, and school type combine into a single percentage. Works for public schools, private schools, and colleges. Supports both imperial and metric units.

### 🌐 [Tip Calculator](https://ganak.app/calculators/tip-calculator/)

Calculate restaurant tips based on bill amount, service quality, and local customs. Split bills evenly among friends, add tax before or after tip, and compare tip percentages (15%, 18%, 20%, 25%). Supports multiple currencies (USD, EUR, GBP, INR, AED). Perfect for dining out, delivery tipping, and travel in different countries.

### 🇮🇳 [Cash Denomination Calculator](https://ganak.app/calculators/india/cash-denomination-calculator/)

Free cash denomination calculator for Indian currency. Enter the count of each note (₹500, ₹200, ₹100, ₹50, ₹20, ₹10) and coin (₹5, ₹2, ₹1) to get the total amount instantly. Perfect for shopkeepers, bank cashiers, accountants, and daily cash tallying. Denomination-wise breakdown with notes and coins summary.

### 🇮🇳 [Electricity Bill Calculator](https://ganak.app/calculators/india/electricity-bill-calculator/)

Free electricity bill calculator for India with approximate state-wise tariff slabs (FY 2024-25) for 19 states. Enter units consumed or select appliances to get an indicative bill breakdown with energy charges, fixed charges, electricity duty, and fuel surcharge. Actual bills vary by DISCOM. Supports domestic and commercial connections for Maharashtra, Delhi, Tamil Nadu, UP, Karnataka, Kerala, and more.

[⬆ Back to contents](#contents)

## 📌 [Food](https://ganak.app/calculators/category/food/)

### 🌐 [ABV Calculator](https://ganak.app/calculators/abv-calculator/)

Free ABV calculator for beer, wine, mead, and cocktails. Calculate alcohol by volume from OG and FG gravity readings in SG, Plato, or Brix. Includes cocktail ABV mixer, ABV to Proof converter, attenuation, calories per serving, and standard drink count. Supports both simple and alternate (high-gravity) formulas.

[⬆ Back to contents](#contents)

---

## Suggest a Calculator

Have an idea for a calculator that should exist? [Open an issue](../../issues) and we'll consider adding it!

## License

[CC0 1.0](https://creativecommons.org/publicdomain/zero/1.0/) — this list is in the public domain.

---

<sub>Last updated: 2026-03-16 · 249 calculators</sub>
